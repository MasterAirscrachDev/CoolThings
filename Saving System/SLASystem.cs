...
{ //the editable class
    ...
    //0 permission can update data
    //1 permission can overwrite data
    ...
    //configure class
    public void Setup(bool debugfile, string projectname, string studioname) 
    { 
        debug = debugfile; projectName = projectname; studioName = studioname;
    }
}
public class SLASystem
{
    ...
    //Save A SaveFile
    public async Task Save(string filename, SaveFile save, string encryptKey = "UseDefaultKey", string subfolder = null)
    {
        //check if the data is valid
        if (!string.IsNullOrEmpty(save.projectName) && !string.IsNullOrEmpty(save.studioName))
        {
            working = true;
            ...
            else if (encryptKey == "UseDefaultKey")
            {
            ...
            if (save.debug) { Console.WriteLine("Data backed up"); }
            working = false;
        }
        else { Console.WriteLine("Project or Studio name is empty!"); }
    }
    //Load A Save File
    public async Task<SaveFile> Load(string filename, SaveFile savein, string encryptKey = "UseDefaultKey", string subfolder = null)
    {
        SaveFile save = new SaveFile(); string json = null; CerData data = new CerData();
        //check if the input savefile is valid
        if (!string.IsNullOrEmpty(savein.projectName) && !string.IsNullOrEmpty(savein.studioName))
        {
            working = true;
            if (File.Exists($"{location}\\{savein.studioName}\\{savein.projectName}\\{subfolder}{filename}.dat"))
            {
                ...
                if (save.debug) { Console.WriteLine("Data loaded"); }
                return save;
            }
            ...
            else if (File.Exists($"{location}\\{savein.studioName}\\{savein.projectName}\\{subfolder}{filename}.datbak"))
            {
                ...
                await Save(filename, save, encryptKey, subfolder); working = false; return save;
            }
            else if (savein.debug) { Console.WriteLine("There is no save data!"); }
            working = false; return null;
        } //if the input SaveFile was invalid
        else { Console.WriteLine("Project or Studio name is not set!"); }
        return null;
    }
}